...
from paper_dl.config import AppConfig, load_config, set_config
from paper_dl.models import SearchQuery
...
def cmd_search(
    ...
    async def _run_search():
        ...
        from paper_dl.metadata.extractor import MetadataExtractor
    ...
    # 展示结果（传入 author 用于显示匹配度列）
    _display_results(papers, fmt, query_author=author)
...
def cmd_download(
    ...
    async def _run_download():
        ...
        if doi:
            ...
            from paper_dl.models import PaperMetadata, AccessStatus
...
def config_init():
    """在当前目录生成默认配置文件 paper_dl.toml。"""
    example = Path(__file__).parent.parent / "paper_dl.toml.example"
    dest = Path("paper_dl.toml")
    if dest.exists():
        overwrite = typer.confirm("paper_dl.toml 已存在，是否覆盖？", default=False)
        if not overwrite:
            raise typer.Exit(0)
    ...
    console.print("[yellow]请编辑 paper_dl.toml，至少填写 unpaywall_email[/yellow]")
...
def _display_results(papers, fmt: str, query_author: Optional[str] = None) -> None:
    if fmt == "json":
    ...
    if show_score:
        table.add_column("匹配度", width=8, justify="center")
    ...
    if show_score:
        console.print(
            "[dim]匹配度说明: [green]≥0.80 高置信[/green]  "
            "[yellow]0.50–0.79 中等[/yellow]  "
            "[red]<0.50 低置信[/red][/dim]"
        )
...
def _do_download(papers, output_dir: str, cfg: AppConfig,
    ...
    async def _run_dl():
        from paper_dl.download.downloader import PaperDownloader
